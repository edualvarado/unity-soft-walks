...
public class MovementXbot : MonoBehaviour
{
    #region Variables
  ...
  /*  public GameObject shoesRight;
    ...
    [Header("Deformable ground material")]
    [Range(1, 10)]
    ...
    [Header("Grass")]
    [Range(0, 1000.0f)]
    public float grassResistance = 500.0f;
    ...
    private void OnAnimatorIK(int layerIndex)
    {
        ...
        /*if (grassState == 1)
        {
            if (anim.GetFloat(rightFootAnimVariableName) == 0.0f)
            {
                cylinderR.transform.Translate(new Vector3(rightFootIKPosition.x - cylinderR.transform.position.x, 0.0f, rightFootIKPosition.z + 0.1f - cylinderR.transform.position.z));
            }
            else if (anim.GetFloat(leftFootAnimVariableName) == 0.0f)
            {
                cylinderR.transform.Translate(new Vector3(leftFootIKPosition.x - cylinderR.transform.position.x, 0.0f, leftFootIKPosition.z + 0.1f - cylinderR.transform.position.z));
            }
        }*/
    ...
    {
        /*Debug.Log(grassState);*/
        //High grass
        if (isGrass)
        {
            grassResistance = 1.0f * Mathf.Pow(10, 5);
            rb.AddForce(grassResistance * Vector3.forward, ForceMode.Force);
        }
        //Low or no grass
        else
        {
    ...
    {
        Vector3 originLow = transform.position;
        Vector3 originMiddle = transform.position + new Vector3(0.0f, 0.5f, 0.0f);
        Vector3 originHigh = transform.position + new Vector3(0.0f, 1.0f, 0.0f);

        if (Physics.Raycast(originHigh, Vector3.forward, 0.5f))
        {
            //Debug.Log("High grass");
            grassState = 3;
        }

        else if (Physics.Raycast(originMiddle, Vector3.forward, 0.5f))
        {
            //Debug.Log("Middle grass");
            grassState = 2;
        }

        else if (Physics.Raycast(originLow, Vector3.forward, 0.5f))
        {
            //Debug.Log("Low grass");
            grassState = 1;
        }
    ...
    IEnumerator writeToMaterial()
    {
        while (true)
        {
            ...
            {
                materials[i].SetVector("_position", charPos);
            }
