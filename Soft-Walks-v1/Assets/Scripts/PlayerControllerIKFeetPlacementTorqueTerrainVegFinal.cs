...
public class PlayerControllerIKFeetPlacementTorqueTerrainVegFinal : MonoBehaviour
{
    #region Variables
    ...
    public float redRightOffsetX;
    public float blueRightOffsetX;
    public float redLeftOffsetY;
    public float blueLeftOffsetY;
    public float redRightOffsetY;
    public float blueRightOffsetY;
    public GameObject redPlayer;
    private PlayerControllerIKFeetPlacementTorqueTerrainVegFinal redScript;
    ...
    [SerializeField] private float tau2;
    ...
    [Range(0, 0.5f)] public float maxHeightThreshold = 0.3f;
    ...
    [Header("Comparison")] // Others
    public bool showGoalLineDebug = false;
    ...
    public int recordFrames = 120;
    public string folder = @"C:\Temp\UnityShots";
    ...
    void Start()
    {
        // OBTAINING OTHER IK GOAL
        if (addOFFSETFOOT)
            redScript = redPlayer.GetComponent<PlayerControllerIKFeetPlacementTorqueTerrainVegFinal>();
        ...
        //sphereCOM = GameObject.CreatePrimitive(PrimitiveType.Sphere);
        //sphereCOM.GetComponent<Collider>().enabled = false;
        //MeshRenderer renderLeftSphere = sphereCOM.GetComponent<MeshRenderer>();
        //renderLeftSphere.enabled = true;
        //sphereCOM.transform.position = anim.bodyPosition;
        //sphereCOM.transform.localScale = new Vector3(0.1f, 0.1f, 0.1f);
    ...
    void Update()
    {
        ...
        if(Input.GetKeyDown(KeyCode.Backspace))
        {
            ...
            string name = string.Format("{0}/shot.png", folder);
            ScreenCapture.CaptureScreenshot(name, 1);
        }

        // Create sphere for the first time.
        //sphereCOM.transform.position = anim.bodyPosition + Vector3.forward * -1f;
        ...
        if (capture)
        {
            ...
            string name = string.Format("{0}/shot {1:D05}.png", folder, shotCount);

            // Capture the screenshot to the specified file.
            if (shotCount % 68 == 0)
                ScreenCapture.CaptureScreenshot(name, 1);

            //float diff = (Time.realtimeSinceStartup - lastTime);
            //float normal = 1f / frameRate;
            //float m = Mathf.Clamp01(normal / diff);
        }
        //if (shotCount >= recordFrames)
        //{
        //    capture = false;
        //}
    ...
    private void CheckFeetAreGrounded()
    {
        ...
        {
            if (Physics.CheckSphere(groundCheckerLeftFoot.position, groundDistanceFoot, ground, QueryTriggerInteraction.Ignore) || Physics.CheckSphere(groundCheckerLeftFootBack.position, groundDistanceFoot, ground, QueryTriggerInteraction.Ignore))
            {
        ...
        {
            if (Physics.CheckSphere(groundCheckerRightFoot.position, groundDistanceFoot, ground, QueryTriggerInteraction.Ignore) || Physics.CheckSphere(groundCheckerRightFootBack.position, groundDistanceFoot, ground, QueryTriggerInteraction.Ignore))
            {
    ...
    private void OnAnimatorIK(int layerIndex)
    {
        ...
        // Only for comparison with multiple characters
        if(showGoalLineDebug)
            Debug.DrawLine(startingLine.transform.position, goalLine.transform.position, Color.blue);
    ...
    private void BalanceCharacter()
    {
        ...
        if ((dotRightFoot <= dotLeftFoot)) //Left foot in front of right foot
        {
            ...
            // Changing the positions to be the IK goals. <- PREVIOUS
            targetRbPosition = Vector3.Dot(rightFootPosition, transform.forward) +
                ((leftFootIKPosition - rightFootIKPosition).magnitude / 2.0f) * Mathf.Cos(slopeAngleChloe * 3.1415f / 180.0f);

            // Changing the positions to be the IK goals and removing the cos component.
            //targetRbPosition = Vector3.Dot(rightFootPosition, transform.forward) +
            //    ((leftFootIKPosition - rightFootIKPosition).magnitude / 2.0f);

            // Changing the positions to be the IK goals and adding the sin component to be the projection on the slope.
            //targetRbPosition = Vector3.Dot(rightFootPosition, transform.forward) +
            //    ((leftFootIKPosition - rightFootIKPosition).magnitude / 2.0f) * Mathf.Cos(slopeAngleChloe * 3.1415f / 180.0f) +
            //    ((leftFootIKPosition - rightFootIKPosition).magnitude / 2.0f) * Mathf.Sin(slopeAngleChloe * 3.1415f / 180.0f);


            // Vector from (0,0) to IK feet in 2D
            //Vector2 left = new Vector2(leftFootIKPosition.z, leftFootIKPosition.y);
            //Vector2 right = new Vector2(rightFootIKPosition.z, rightFootIKPosition.y);
            Vector2 left = new Vector2(leftFootIKPosition.x, leftFootIKPosition.y);
            Vector2 right = new Vector2(rightFootIKPosition.x, rightFootIKPosition.y);
            Debug.DrawLine(Vector2.zero, new Vector2(leftFootIKPosition.x, leftFootIKPosition.y), Color.red);
            Debug.DrawLine(Vector2.zero, new Vector2(rightFootIKPosition.x, rightFootIKPosition.y), Color.green);

            Vector2 p_target = right + (left - right) / 2.0f;
            Debug.DrawLine(Vector2.zero, p_target, Color.blue);

            //Vector2 p_com = new Vector2(anim.bodyPosition.z, anim.bodyPosition.y);
            Vector2 p_com = new Vector2(anim.bodyPosition.x, anim.bodyPosition.y);
            Debug.DrawLine(Vector2.zero, p_com, Color.cyan);

            // New
            Vector2 footbehind_com = p_com - right;
            Vector2 footbehind_target = p_target - right;
            float distance_footbehind_proj = Vector2.Dot(footbehind_target, footbehind_com) / footbehind_target.magnitude;
            float diff_proj = footbehind_target.magnitude - distance_footbehind_proj;

            //float diff = (p_com - p_target).magnitude;
            //float sign = Math.Sign(p_target[0] - p_com[0]);
            float diff = p_com[0] - p_target[0];

            //Debug.Log("Diff: " + sign * diff);
            ...
            if (showPDDebug)
            {
                ...
                */
                ...
                Debug.DrawLine(leftFootIKPosition, rightFootIKPosition, Color.red);
                Debug.DrawLine(Vector3.zero, rightFootIKPosition + ((leftFootIKPosition - rightFootIKPosition) / 2.0f), Color.blue);
            }
        ...
        else //Right foot in front of left foot
        {
            ...
            // Changing the positions to be the IK goals. <- PREVIOUS
            targetRbPosition = Vector3.Dot(leftFootPosition, transform.forward) +
                ((rightFootIKPosition - leftFootIKPosition).magnitude / 2.0f) * Mathf.Cos(slopeAngleChloe * 3.1415f / 180.0f);

            // Changing the positions to be the IK goals and removing the cos component.
            //targetRbPosition = Vector3.Dot(leftFootPosition, transform.forward) + 
            //    ((rightFootIKPosition - leftFootIKPosition).magnitude / 2.0f);

            // Changing the positions to be the IK goals and adding the sin component to be the projection on the slope.
            //targetRbPosition = Vector3.Dot(leftFootPosition, transform.forward) +
            //    ((rightFootIKPosition - leftFootIKPosition).magnitude / 2.0f) * Mathf.Cos(slopeAngleChloe * 3.1415f / 180.0f) + 
            //    ((rightFootIKPosition - leftFootIKPosition).magnitude / 2.0f) * Mathf.Sin(slopeAngleChloe * 3.1415f / 180.0f);
            ...
            Vector2 p_target = left + (right - left) / 2.0f;
            Debug.DrawLine(Vector2.zero, p_target, Color.blue);
            ...
            // New
            Vector2 footbehind_com = p_com - left;
            Vector2 footbehind_target = p_target - left;
            float distance_footbehind_proj = Vector2.Dot(footbehind_target, footbehind_com) / footbehind_target.magnitude;
            float diff_proj = footbehind_target.magnitude - distance_footbehind_proj;
            ...
            if (showPDDebug)
            {
                ...
                */
                ...
                Debug.DrawLine(leftFootIKPosition, rightFootIKPosition, Color.blue);
                Debug.DrawLine(Vector3.zero, leftFootIKPosition + ((rightFootIKPosition - leftFootIKPosition) / 2.0f), Color.red);
        ...
        else if (heightGrass == 2 && isGrass && addTorque)
        {
        ...
        else if (heightGrass == 1 && isGrass && addTorque)
        {
    ...
    void MoveFeetToIKPoint(AvatarIKGoal foot, Vector3 positionIKHolder, Quaternion rotationIKHolder, ref float lastFootPositionY)
    {
        ...
        if (positionIKHolder != Vector3.zero)
        {
            ...
            if(addOFFSETFOOT)
            {
                //Debug.Log("redLeft: " + leftRedFoot.position.x);
                //Debug.Log("blueLeft: " + leftBlueFoot.position.x);

                // RED
                if (foot == AvatarIKGoal.LeftFoot && targetIKPosition.z > 0)
                {
                    //Debug.Log("targetIKPosition RED: " + targetIKPosition);
                    //Debug.Log("targetIKPosition RED X: " + targetIKPosition.x);
                    redLeftOffsetX = targetIKPosition.x;
                    redLeftOffsetY = targetIKPosition.y;
                }

                // RED
                if (foot == AvatarIKGoal.RightFoot && targetIKPosition.z > 0)
                {
                    //Debug.Log("targetIKPosition RED: " + targetIKPosition);
                    //Debug.Log("targetIKPosition RED X: " + targetIKPosition.x);
                    redRightOffsetX = targetIKPosition.x;
                    redRightOffsetY = targetIKPosition.y;
                }

                // BLUE
                if (foot == AvatarIKGoal.LeftFoot && targetIKPosition.z < 0)
                {
                    blueLeftOffsetX = targetIKPosition.x;
                    blueLeftOffsetY = targetIKPosition.y;
                    
                    float offsetToAddLeftX = blueLeftOffsetX - redScript.redLeftOffsetX;
                    float newBlueLeftTargetX = (blueLeftOffsetX - offsetToAddLeftX);
                    
                    float offsetToAddLeftY = blueLeftOffsetY - redScript.redLeftOffsetY;
                    float newBlueLeftTargetY = (blueLeftOffsetY - offsetToAddLeftY);

                    /*
                    Debug.Log("======");
                    Debug.Log("Initial Left IK Position (BLUE): " + blueLeftOffset);
                    Debug.Log("Left IK Position (RED): " + redScript.redLeftOffset);
                    Debug.Log("Offset: " + offsetToAddLeft);
                    Debug.Log("Final Left IK Position (BLUE): " + newBlueLeftTargetX);
                    Debug.Log("======");
                    */

                    targetIKPosition = new Vector3(newBlueLeftTargetX, newBlueLeftTargetY, targetIKPosition.z);
                }

                if (foot == AvatarIKGoal.RightFoot && targetIKPosition.z < 0)
                {
                    blueRightOffsetX = targetIKPosition.x;
                    blueRightOffsetY = targetIKPosition.y;

                    float offsetToAddRightX = blueRightOffsetX - redScript.redRightOffsetX;
                    float newBlueRightTargetX = (blueRightOffsetX - offsetToAddRightX);

                    float offsetToAddRightY = blueRightOffsetY - redScript.redRightOffsetY;
                    float newBlueRightTargetY = (blueRightOffsetY - offsetToAddRightY);

                    /*
                    Debug.Log("======");
                    Debug.Log("Initial Right IK Position (BLUE): " + blueRightOffset);
                    Debug.Log("Right IK Position (RED): " + redScript.redRightOffset);
                    Debug.Log("Offset: " + offsetToAddRight);
                    Debug.Log("Final Right IK Position (BLUE): " + newBlueRightTargetX);
                    Debug.Log("======");
                    */

                    targetIKPosition = new Vector3(newBlueRightTargetX, newBlueRightTargetY, targetIKPosition.z);
                }
            }
